...
BeholdRC::BeholdRC() {
  ...
  if ( hLib ) {
    ...
    if ( !pGetCardCount || !pOpenCard || !pGetRemoteCode || !pUnInit ) {	// GetRemoteCodeEx may be absent in library. Don't check for it.
      FreeLibrary (hLib);
      hLib = NULL;
    }
  }
...
int BTV_GetIStatus (void) {
  if ( rc.hLib==NULL )  return 0;	// Library not found.
  if ( !rc.init )  return 1;	// WDM device not selected.
  return 2;	// OK.
}
...
ULONG BTV_GetRCCodeEx (void) {
  ULONG  code;
  if ( rc.hLib && rc.init ) {
    if ( rc.pGetRemoteCodeEx )  return (*rc.pGetRemoteCodeEx)();
    code = (*rc.pGetRemoteCode)();	// The BeholdTV software has old BeholdRC.dll. Emulate extended function
    if ( code<255 )  return 0x866B0000 | (code<<8) | (~code&0xFF);	// Combine full NEC-standard code
  }
  return 0;
}
